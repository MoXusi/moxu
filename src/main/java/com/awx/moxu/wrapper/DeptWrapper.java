...
public class DeptWrapper extends BaseEntityWrapper<Dept, DeptVO> {
	...
	static {
		deptService = SpringUtil.getBean(DeptService.class);
	}
	...
	public List<INode> listNodeVO(List<Dept> list) {
		List<INode> collect = list.stream().map(dept -> BeanUtil.copyProperties(dept, DeptVO.class)).collect(Collectors.toList());
		return ForestNodeMerger.merge(collect);
	}
	public IPage<INode> pageVOINode(IPage<Dept> pages) {
		List<INode> iNodes = this.listNodeVO(pages.getRecords());
		IPage<INode> pageVo = new Page(pages.getCurrent(), pages.getSize(), pages.getTotal());
		pageVo.setRecords(iNodes);
		return pageVo;
	}
}
